...
const EmpArchive = () => {
  ...
  useEffect(() => {
    const storedEmpArchive =
      JSON.parse(localStorage.getItem(LOCAL_KEYS.EMP_ARCHIVE)) || [];
    const empTrashData =
      JSON.parse(localStorage.getItem(LOCAL_KEYS.EMP_TRASH)) || [];

    const cleanArchive = storedEmpArchive.filter(
      (item) => !empTrashData.find((trash) => trash.id === item.id)
    );
  ...
  const handleRestore = (entry) => {
    restoreSingleItem({
      ...
      localKeySource: LOCAL_KEYS.EMP_ARCHIVE,
